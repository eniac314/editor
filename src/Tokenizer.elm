...
tokenString res =
    case res of
        ...
        Ok ( [], _ ) ->
            Err "tokenString: Invalid String literal"
        ...
        Ok ( c :: cs, rest ) ->
            let
                ...
                val =
                    foldr (\c s -> cons (.ch c) s) "" (c :: cs)
            in
                Ok ( (Token ("\"" ++ val ++ "\"") ch ln), rest )
...
tokenCssComment res =
    case res of
        ...
        Ok ( [], _ ) ->
            Err "tokenCssComment: Invalid css comment"
...
getTokens xs =
    let
        ...
        tString xs =
            String.fromList (List.map (.ch) xs)
        ...
        helper xs acc =
            case xs of
                ...
                x :: xs ->
                    if (isTokenChar (.ch x)) then
                        ( tokenize acc
                            ++ [ Token (fromChar (.ch x)) (.chp x) (.lnp x) ]
                        , xs
                        )
                    else if (isSpace (.ch x)) then
                        ( tokenize acc, xs )
                    else if (.ch x == '"') then
                        ( tokenize acc, x :: xs )
                    else
...
tokenizer_ : CharMeta -> Result String (List Token)
tokenizer_ cs =
    let
        hasCommentStart s =
            case s of
    ...
    in
        case cs of
            ...
            x :: xs ->
                if hasCommentStart cs then
                    case tokenCssComment (getCssComment <| List.drop 2 cs) of
                        Err s ->
                            Err s

                        Ok ( t, rest ) ->
                            case tokenizer_ rest of
                                Err s ->
                                    Err s

                                Ok ts_ ->
                                    Ok (ts_)
                else if hasStringLitStart cs then
